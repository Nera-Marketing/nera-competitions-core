...
// Pre-compute sales percentage once (shared across all prize threshold checks).
$_total_tickets  = intval( $product->get_lty_maximum_tickets() );
$_pct_sold       = ( $_total_tickets > 0 )
	? ( intval( $product->get_purchased_ticket_count() ) / $_total_tickets ) * 100
	: 100; // if total is 0 treat as fully sold so prizes aren't locked
...
foreach ($instant_winner_ids as $instant_winner_id) {
  ...
  if (!isset($prizes_grouped[$key])) {
    $prizes_grouped[$key] = [
      ...
      'activation_threshold' => intval( get_post_meta( $instant_winner_id, 'lty_activation_threshold', true ) ),
      'is_won'               => false,
      'total_count'          => 0,
      'won_count'            => 0,
      'winners'              => [],
...
<div class="instant-wins-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" x-data="{ openId: null }">
 ...
 foreach ($prizes_grouped as $prize_key => $prize_data):
   ...
   $threshold        = intval( $prize_data['activation_threshold'] );
   $is_locked        = ( $threshold > 0 && $_pct_sold < $threshold );
   $display_limit    = 4;
   ...
   ?>
		...
		<!-- Prize Card -->
		<div
			class="instant-win-card bg-surface rounded-2xl shadow-card hover:shadow-card-hover transition-all duration-300 overflow-hidden <?php echo $is_locked
     ? 'prize-locked'
     : ( $is_won ? 'prize-won' : 'prize-available' ); ?>"
		>
			<!-- Card Image with Coin Badge Overlay -->
			<div class="card-image-wrapper relative">
				<?php if ($prize_image): ?>
					<div class="prize-image aspect-[4/3] w-full overflow-hidden bg-gray-100 <?php echo $is_locked ? 'opacity-60' : ''; ?>">
						<?php echo wp_kses_post($prize_image); ?>
					</div>
				<?php else: ?>
					<div class="prize-image aspect-[4/3] w-full overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center <?php echo $is_locked ? 'opacity-60' : ''; ?>">
				...
				<?php if ($is_locked): ?>
					<!-- Lock Badge Overlay -->
					<div class="coin-badge absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-full bg-gradient-to-br from-gray-400 via-gray-500 to-gray-600 shadow-2xl flex items-center justify-center border-4 border-white">
						<div class="text-center">
							<span class="material-symbols-outlined text-white text-2xl mb-1 block">lock</span>
							<span class="text-white font-bold text-xs uppercase tracking-wide">Locked</span>
						</div>
					</div>
				<?php else: ?>
			...
			<?php if ($is_locked): ?>
				<!-- Locked Prize - Static Card (Not Yet Active) -->
				<div class="prize-info p-5 border-t border-gray-100">
					<div>
						...
						<p class="text-xs text-text-secondary mt-2">
							Unlocks once <?php echo esc_html( $threshold ); ?>% of tickets are sold
						</p>
					</div>
				</div>
