<template>
  ...
  >
    ...
    <component
      ...
      :aria-controls="isInteractive ? `prize-card-body-${prize.key}` : null"
    >
      <div class="flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-2 flex-wrap mb-3">
            ...
            <template v-else>
              ...
              <template v-if="prize.totalCount != null">
                ...
                >
                  {{ remainingCount }} / {{ prize.totalCount }} To Be Won
                </span>
              </template>
              <template v-else>
          ...
          <!-- Per-Prize Progress Bar — only when fully won and there's something to chart -->
          <template v-if="isFullyWon && prize.totalCount != null && prize.totalCount > 0">
...
/**
 * Prize Card — default Instant Win display mode.
 *
 * Accordion opens whenever there are winners to reveal (including partially
 * won categories). Fully won cards show All Won; otherwise Available +
 * remaining "X / Y To Be Won".
 */
...
const isFullyWon = computed(() => {
  // Prefer explicit isWon from container when present; fall back to stock math.
  if (props.prize.isWon === true) return true;
...
// Interactive accordion only when there are winners to reveal.
